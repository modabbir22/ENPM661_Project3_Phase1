...
# def combined_plot_animation(start_node, goal_node, path_x, path_y, visited_nodes, environment):
#     fig, ax = plt.subplots()
...
#     def update(frame):
#         # Update explored nodes (gradually)
#         if frame < total_frames - len(path_x) + 1:  # Show explored nodes until path_line starts
#             end_index = min((frame + 1) * max_nodes_per_frame, len(visited_nodes))
#             x_coords, y_coords = zip(*[(wp[0], wp[1]) for wp in visited_nodes[:end_index]])
#             explored_nodes.set_data(x_coords, y_coords)
#             print("frame_no.:", frame)
        
#         # Update path line gradually (starting from frame before path_line)
...
#     total_points = len(visited_nodes)  # Not used in this version
...
#     frames = (len(visited_nodes) + 99) // max_nodes_per_frame
